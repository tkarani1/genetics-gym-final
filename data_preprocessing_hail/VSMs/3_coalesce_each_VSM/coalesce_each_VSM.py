...
from resources.paths import JULIA_VSM_LINKER_TABLE_PATHS, JULIA_VSM_NAMES, WRITE_COALESCED_JULIA_VSM_BASE
from resources.constants import JULIA_SCORE_FIELDS, JULIA_SCORE_FIELDS_ALL
...
methods = [method for method in JULIA_VSM_NAMES.keys()]
# methods = ['DBNSFP4', 'GMVP', 'PHYLOP', 'SIFT']
...
#         write_path_base = "gs://genetics-gym-not-public/Trisha/Julia_tables/" + VSM_TABLE_NAMES[method] + '_' + col
...
## Julia version
for method in methods:    
    ht_path = JULIA_VSM_LINKER_TABLE_PATHS[method]
    if not hl.hadoop_exists(ht_path):
        print(f"Linker table not found: {method}")
        continue
    write_path = WRITE_COALESCED_JULIA_VSM_BASE + JULIA_VSM_NAMES[method]
    if hl.hadoop_exists(write_path):
        print(f"Coalesced table already exists: {method}")
        continue

    ht = coalesce_VSM(ht_path, ['locus', 'alleles'], JULIA_SCORE_FIELDS_ALL[method])
    ht = ht.checkpoint(write_path, overwrite=True)
    write_parquet_from_ht(ht, write_path + '.parquet', convert_locus_alleles=True)
