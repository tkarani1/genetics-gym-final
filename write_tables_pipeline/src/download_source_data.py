...
``data/raw_data/input_data_locations.json`` and downloads them into
``data/raw_data/scores`` (for ``score_tables``),
``data/raw_data/evals`` (for ``eval_tables``),
``data/raw_data/linker`` (for the standalone ``linker_table``), or
``data/raw_data/filters`` (for ``filter_tables``).
...
directory::
    ...
    python download_source_data.py                       # download everything
...
Selection semantics:
    * With no selection flags, everything is downloaded (the default behavior).
...
LOCATIONS_JSON = RAW_DATA_DIR / "input_data_locations.json"
...
def main() -> int:
    ...
    # Default (no flags) downloads everything. If the user specifies any
    # group, the unspecified groups default to 'none'.
    any_flag = (
    ...
    jobs = collect_sources(
        data, score_selection, eval_selection, filter_selection, include_linker
    )
    if not jobs:
        print("Nothing selected to download. Use --score-tables / --eval-tables / "
              "--filter-tables with 'all', 'none', or specific subcategory keys.")
        return 0
