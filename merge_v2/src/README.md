...
```
merge_v2/
├── data_config/                             # tracked pipeline config (manifests)
...
├── data/                                    # gitignored (large data tables)
│   ├── raw_data/
...
│       │   ├── pairwise/                     # anchor-based pairwise tables (per pair)
│       │   ├── pairwise_consolidated/        # one consolidated table per pairwise flavor
...
The pairwise score tables can be turned into analysis tables in **two ways**,
selected via [`config.json`](#configjson):
...
- **(ii) consolidated** — enable `create_pairwise_consolidated_score_tables` and
  ...
  consolidation step first collapses each flavor's per-pair files into a single
  `scores/pairwise_consolidated/pairwise_<flavor>_consolidated.parquet`, and the
  analysis step then writes one analysis table *per flavor* under
...
pairwise route — see below). The `--exclude`-style options are still parsed by
the pipeline so the audit's recorded inputs match what the step actually
consumes. The three `*_obs_exp_mis.parquet` eval sources were previously
excluded from the variant eval merge (`--exclude _obs_exp_mis`) because they were
wrong upstream; they have since been corrected (filtered to missense via the
linker) and are now **included**, so that guard has been removed. If
`config.json` is missing or `--no-config` is given, the pipeline falls back to
...
`../data_config/score_input_data.json`, the script projects the
...
`../data_config/evals_input_data.json` into a single wide
...
> The three `*_obs_exp_mis.parquet` sources were originally **excluded** from
> `variant_evals_all.parquet` (`--exclude _obs_exp_mis`) because those inputs were
> wrong upstream — see `../.agent_reports/obs_exp_mis_preprocessing_assessment.md`.
> They have since been corrected (filtered to actual missense variants via an
> inner join with the variant→gene linker) and are now **included** in the merge,
> so the exclusion has been removed from [`config.json`](#configjson).
...
Collapses the **per-pair** files written by `create_pairwise_score_tables.py`
into a **single** wide table per flavor, writing
`../data/processed_data/scores/pairwise_consolidated/pairwise_{raw,pre,post}_consolidated.parquet`.
This is the score-table input to the `pairwise_consolidated` group of
`create_analysis_tables.py`, so that a single analysis table can be built per
flavor instead of one per pair (see
[the two pairwise routes](#two-routes-for-the-pairwise-analysis-tables)).

For each flavor the per-pair files are combined with a **full outer join** on the
key columns, so the consolidated row universe is exactly the union of the pairs —
every key where the anchor is present together with at least one non-anchor
score. The script is **self-contained**: its only inputs are the per-pair files
under `pairwise/`, and the anchor and non-anchor scores are discovered from the
file names / schema (so it works for any anchor and any set of pairs).

### Column / collision handling

Within a flavor every per-pair file shares the key columns and the same *anchor*
column name, and carries one unique *non-anchor* column. Non-anchor columns are
already unique across the files and are carried through unchanged. The anchor
column collides across all files, and how it is resolved is exactly what
distinguishes the flavors:

| Flavor | Anchor column | Why |
| ------ | ------------- | --- |
| `raw` / `pre` | **coalesced** into one column (original name kept) | the anchor value for a key is identical in every pair (the raw value, resp. the precomputed *outer* percentile) |
| `post` | **every copy kept**, each renamed `{anchor_col}_{anchor}_x_{non_anchor}` (e.g. `polyphen_polyphen_anchor_post_percentile_polyphen_x_AM`) | the anchor's percentile is `CUME_DIST` recomputed over **each pair's own intersection**, so the value differs per pair |

So `raw`/`pre` consolidate to `keys + 1 anchor + N non-anchor` columns, while
`post` keeps `keys + N anchor + N non-anchor` columns (one anchor percentile per
pair, all preserved and uniquely named).

### Engine / memory strategy

Same DuckDB out-of-core philosophy as the upstream scripts. The per-pair files
are combined with a **sequential** full-outer join, materializing one stage table
at a time (each stage drops the previous one) so peak memory stays bounded;
DuckDB spills to `--temp-dir` under `--memory-limit`. Existing consolidated
outputs are **skipped** (resumable) unless `--overwrite` is set.

### Usage

```bash
# From merge_v2/src (requires create_pairwise_score_tables.py to have run first)
python create_pairwise_consolidated_score_tables.py --variant                 # raw + pre + post
python create_pairwise_consolidated_score_tables.py --variant --modes post    # subset of flavors
python create_pairwise_consolidated_score_tables.py --variant --anchor cadd   # match the anchor used upstream
python create_pairwise_consolidated_score_tables.py --variant --memory-limit 24GB --threads 8
python create_pairwise_consolidated_score_tables.py --variant --dry-run       # print the plan and exit
```

### Behavior notes

- `--anchor` must match the anchor used by `create_pairwise_score_tables.py`
  (default `polyphen`); it is validated against the per-pair file schema.
- `--modes` selects any subset of `raw` / `pre` / `post` (default: all);
  `--variant` / `--ensg` restrict the dataset (default: both).
- `--pairwise-dir` / `--output-dir` override the input/output roots; the DuckDB
  spill dir defaults to `<output dir>/.duckdb_spill` and is removed on completion.
- The run prints, per flavor, the consolidated row × column counts.
