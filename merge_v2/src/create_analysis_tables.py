...
* ``pairwise`` -- every file under ``scores/pairwise/{pairwise_raw,pairwise_pre,
  ...
  ``full_analysis_tables/pairwise/<flavor>/``. This is the *individual-pair*
  analysis: one analysis table per anchor/non-anchor pair per flavor.

* ``pairwise_consolidated`` -- each ``scores/pairwise_consolidated/*.parquet``
  table (one per flavor, produced by
  ``create_pairwise_consolidated_score_tables.py``) LEFT-JOINed onto
  ``variant_evals_all.parquet`` on the variant key. Outputs go under
  ``full_analysis_tables/pairwise_consolidated/``. This is the *consolidated*
  alternative to the ``pairwise`` group: a single analysis table per flavor
  instead of one per pair. Pick ``pairwise`` **or** ``pairwise_consolidated``
  depending on which downstream representation you want.
...
# Consolidated pairwise score tables (one per flavor) live flat in this subdir.
PAIRWISE_CONSOLIDATED_SUBDIR = "pairwise_consolidated"
...
def build_jobs(
    ...
    if "pairwise_consolidated" in groups:
        cons_dir = scores_dir / PAIRWISE_CONSOLIDATED_SUBDIR
        if cons_dir.is_dir():
            for p in sorted(cons_dir.glob("*.parquet")):
                jobs.append(
                    Job(
                        input_path=p,
                        out_dir=out_dir / PAIRWISE_CONSOLIDATED_SUBDIR,
                        joins=[EvalJoin("ve", variant_evals, VARIANT_KEYS)],
                    )
                )
