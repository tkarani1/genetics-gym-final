...
8. ``created_variant_scores_filtered_tables.py`` -> scores/filtered/*_filtered.parquet
   (needs outer_pre_percentile + linker + filters)
9. ``create_analysis_tables.py``             -> full_analysis_tables/** (joins scores x evals)
...
Step 3 is run with ``--exclude _obs_exp_mis``: those variant-level obs/exp
sources are a known upstream preprocessing bug (they enumerate ~all SNVs across
full transcripts, not possible-missense CDS variants) and would inflate
...
DEFAULT_STEPS = [
    ...
    {"name": "create_variant_eval_all_table", "script": "create_variant_eval_all_table.py",
     "args": ["--exclude", "_obs_exp_mis"], "passthrough": _DUCK, "enabled": True},
...
def step_io_map() -> dict[str, dict[str, list[str]]]:
    ...
    return {
        "download_source_data": {
            "inputs": ["data/raw_data/input_data_locations.json"],
            "outputs": [
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-row-counts", action=argparse.BooleanOptionalAction,
